...
if response.status_code != 200:
    ...
    # response.text는 방금 검색한 웹사이트의 코드를 주고, 우리는 그걸 beautiful soup에게 준다.
    # 웹 사이드의 내용으로 bs를 호출하면, 우리는 그 코드 안에서 검색을 해 볼 수 있다 .
    jobs = soup.find_all('section', class_="jobs") # class의 이름이 jobs인 섹션을 찾아보기.
    for job_section in jobs:
        job_posts = job_section.find_all('li')
        job_posts.pop(-1) # 제일 마지막 줄의 view all은 그냥 버튼이라 읽고 싶지 않아서 마지막 항목을 지웠다.
        ...
        for post in job_posts:
            ...
            company, kind, region = anchor.find_all('span', class_="company") # 우리가 만든 변수에 찾은 내용을 넣고싶다고 파이선에게 알려주기

            title = anchor.find('span', class_='title')
            # find 는 하나의 항목만 준다. /  find all은 전부 준다
    ...
    # 출력 결과 : Trustworthy Full-Time Anywhere in the World Full Stack Software Engineer (React / Python)
        ...
        } # 이 짓은 section의 모든 list에서 실행되므로, for문 밖에 저장하지 않으면 사라진다.
